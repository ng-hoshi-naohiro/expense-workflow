...
expense-workflow/
├── mock/                    # プロトタイプ（モック）一式
│   ├── app_v3.0/            # 最新版（GitHub Pages で公開中）
│   │   ├── src/
...
│   │   ├── package.json
│   │   └── project_plan.md
│   ├── app_v2.0/            # 旧バージョン（参照用、デプロイ対象外）
│   ├── app_v1.0/            # 旧バージョン（参照用、デプロイ対象外）
│   └── doc/                 # モック作成のための資料（画面仕様書・ワイヤーフレーム）
└── doc/                     # 案件全体の資料（開発プロセス記録・引継ぎ資料）
```
...
詳細は [`mock/app_v3.0/project_plan.md`](mock/app_v3.0/project_plan.md) を参照してください。

## デプロイ

`main` ブランチへの push をトリガーに、[.github/workflows/deploy-pages.yml](.github/workflows/deploy-pages.yml) が `mock/app_v3.0/` をビルドし GitHub Pages に自動デプロイします。
